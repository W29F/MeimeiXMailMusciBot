...
from MeiMeiXMail import SUDOERS, app, db_mem, userbot
from MeiMeiXMail.Database import get_active_chats, is_active_chat
from MeiMeiXMail.Decorators.checker import checker, checkerCB
from MeiMeiXMail.Inline import primary_markup
...
@app.on_message(filters.command("meimeijoin") & filters.user(SUDOERS))
async def basffy(_, message):
    if len(message.command) != 2:
        await message.reply_text(
            "**Usage:**\n/meimeijoin [Chat Username or Chat ID]"
        )
        return
...
@app.on_message(filters.command("leavemeimei") & filters.user(SUDOERS))
