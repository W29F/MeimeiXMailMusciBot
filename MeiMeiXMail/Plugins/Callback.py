...
from MeiMeiXMail import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
from MeiMeiXMail.Core.PyTgCalls import MeimeiXMail, Queues
from MeiMeiXMail.Core.PyTgCalls.Converter import convert
from MeiMeiXMail.Core.PyTgCalls.Downloader import download
from MeiMeiXMail.Database import (_get_playlists, delete_playlist, get_playlist,
                            get_playlist_names, is_active_chat, save_playlist)
from MeiMeiXMail.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from MeiMeiXMail.Decorators.admins import AdminRightsCheckCB
from MeiMeiXMail.Decorators.checker import checkerCB
from MeiMeiXMail.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup)
from MeiMeiXMail.Utilities.changers import time_to_seconds
from MeiMeiXMail.Utilities.chat import specialfont_to_normal
from MeiMeiXMail.Utilities.paste import isPreviewUp, paste_queue
from MeiMeiXMail.Utilities.theme import check_theme
from MeiMeiXMail.Utilities.thumbnails import gen_thumb
from MeiMeiXMail.Utilities.timer import start_timer
from MeiMeiXMail.Utilities.youtube import get_yt_info_id
...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await MeimeiXMail.pytgcalls.pause_stream(chat_id)
    ...
    if command == "resumecb":
        ...
        await MeimeiXMail.pytgcalls.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "stopcb":
        ...
        await MeimeiXMail.pytgcalls.leave_group_call(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            if str(finxx) != "raw":
                ...
                await MeimeiXMail.pytgcalls.change_stream(
                    chat_id,
                    InputStream(
                        InputAudioStream(
                            raw_path,
    ...
    if not _playlist:
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                raw_path = await convert(downloaded_file)
                try:
                    await MeimeiXMail.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                raw_path,
